...
def calculate_costald_density(composition_mol_pct: dict, temperature_k: float = 118.15, pressure_bar_a: float = 1.20) -> dict:
    """
    Calculates liquid LNG density using Hankinson-Brobst-Thomson (COSTALD) correlation.
    ...
    """
    # Sum mole percentages
    sum_pct = sum(composition_mol_pct.values())
    if sum_pct <= 0:
        sum_pct = 100.0
    
    # Normalize mole fractions for calculation logic
    ...
    V_delta = ((-0.81446 + 1.43907 * (tau**(1/3)) - 0.81446 * (tau**(2/3)) + 0.190454 * tau) 
               / (1.52816 * (tau**(1/3)) if tau > 0 else 1.0)) * 0.044
